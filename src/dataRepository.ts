...
export class StorageAdapter implements IStorageAdapter {
    ...
    async storeData(data: Response): Promise<void> {
        const textData = await this.responseToText(data); // Convert Response object to text
        const jsdomData = this.textToJSDOM(textData); // Convert text data to JSDOM object
        const jsonData = this.jsDomdDataToJSON(jsdomData); // Convert JSDOM data to JSON format
        this.adaptee.storeData(jsonData); // Store JSON data using the adaptee
    }
}
